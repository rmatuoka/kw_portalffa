class UserMailer < ActionMailer::Base
  ...
  def order_start(order)
    @order = order
    @order_itens = OrderTicket.all(:conditions => ['order_id = ?', @order.id])    
    mail(:to=>"compras@festadasfloresdeatibaia.com.br", :bcc => "bsilva@korewa.com.br, rmatuoka@korewa.com.br", :subject => @order.user.name + " - Novo Pedido através do Site")
  end
  ...
  def order_completed(order)
    @order = order
    @order_itens = OrderTicket.all(:conditions => ['order_id = ?', @order.id])    
    mail(:to=>"compras@festadasfloresdeatibaia.com.br", :bcc => "bsilva@korewa.com.br, rmatuoka@korewa.com.br", :subject => @order.user.name + " - Pagamento Efetuado")    
  end
end
